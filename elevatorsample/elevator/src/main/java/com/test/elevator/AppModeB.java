...
public class AppModeB extends Helper{
	...
	static ArrayList<Node> mergedNodes = new ArrayList<Node>();
	...
	private static void computedStepsFromNode(Node node) {
		...
		for (Edge edge : node.edges) {
			Node currentNode = edge.end;
			if (!currentNode.isVisted()) {
				currentTraversePath.add(currentNode);
				currentIteration += edge.weight + Math.abs(currentNode.start - currentNode.end);
				edge.end.setVisited(true);
				computedStepsFromNode(currentNode);
			}
	...
	private static void populateEdges() {
		mergedNodes.addAll(upward);
		mergedNodes.addAll(downward);

		for (Node node : mergedNodes) {
			for (Node currentNode : mergedNodes) {
				if (node == currentNode) {
					continue;
				}
